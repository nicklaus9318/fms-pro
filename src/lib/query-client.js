...
export const prefetchCoreData = () => {
  ...
  qc.prefetchQuery({
    queryKey: ['players'],
    queryFn: async () => {
      const { data } = await supabase.from('players').select('id,first_name,last_name,role,age,overall_rating,goals,assists,mvp_count,yellow_cards_accumulated,player_status,team_id,id_sofifa,photo_url,status').eq('status', 'approved');
      return data || [];
    },
    staleTime: 5 * 60 * 1000,
  });
